...
public class Server {
	...
	public static void main(String args[]) throws Exception {
		...
		int clientNumber = 0;
		System.out.println("The server is up and running on port: " + portNumber + "\n" + " waiting for client");

		ServerSocket serverSocket = new ServerSocket(portNumber);

		try {
			while (true) {
				new service(serverSocket.accept(), clientNumber++).start();
			}
		} finally {
			serverSocket.close();
		}
	...
	private static class service extends Thread {
		...
		public void run() {
			try {
				...
				String path = "Server";
				path += requestParam[1].substring(1);
				...
				if (request.contains("GET")) {
					...
					try {
						...
						System.err.println("Missing file from the client: " + e.getMessage());
						br.close();
					}
				} else {
